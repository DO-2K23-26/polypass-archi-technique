...
sequenceDiagram
    ...
    Frontend->>SearchService: GET /search/credentials?title={searchString}
    SearchService->>SearchEngine: Query credentials by title
    SearchEngine-->>SearchService: Return matching credentials ids
    SearchService->>AgregationService: GET /credentials/{credentialId}
    AgregationService-->>SearchService: Return credentials
    SearchService-->>Frontend: Return credentials list
...
sequenceDiagram
    ...
    Frontend->>SearchService: GET /search/folders?name={folderName}
    SearchService->>SearchEngine: Query folders by name
    SearchEngine-->>SearchService: Return matching folders
    SearchService-->>Frontend: Return folders list
...
sequenceDiagram
    ...
    Frontend->>SearchService: GET /search/credentials?title={searchString}&folder_id={selectedFolderId}
...
sequenceDiagram
    ...
    Frontend->>SearchService: GET /search/tags?name={searchString}
    SearchService->>SearchEngine: Query tags by name
    SearchEngine-->>SearchService: Return matching tags
    SearchService-->>Frontend: Return tags list
    Frontend-->>User: Display tags for selection
    User->>Frontend: Select tags
    Frontend->>SearchService: GET /search/credentials?tag_ids={selectedTagIds}
